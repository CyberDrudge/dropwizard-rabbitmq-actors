...
public class RMQMetricObserver extends RMQObserver {
    ...
    public <T> T executePublish(PublishObserverContext context, Function<HashMap<String, Object>, T> supplier, HashMap<String, Object> headers) {
        log.info("Inside ConsumerMetricObserver.executePublish with headers: {}", headers);
        ...
        log.info("Executing ConsumerMetricObserver.executePublish with headers: {}", headers);
    ...
    public <T> T executeConsume(PublishObserverContext context, Supplier<T> supplier,
        ...
        log.info("Inside ConsumerMetricObserver.executeConsume with headers: {}", headers);
        if (!MetricUtil.isMetricApplicable(rmqConfig.getMetricConfig(), context.getQueueName())) {
            return proceedConsume(context, supplier, headers);
        }
        log.info("Executing ConsumerMetricObserver.executeConsume with headers: {}", headers);
        val metricData = getMetricData(context);
        metricData.getTotal().mark();
        val timer = metricData.getTimer().time();
        try {
